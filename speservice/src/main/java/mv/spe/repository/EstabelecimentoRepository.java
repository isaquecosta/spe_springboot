...
public interface EstabelecimentoRepository extends JpaRepository<Estabelecimento, Long> {
    ...
    @Query(
            value =
                    "SELECT     new mv.spe.service.dto.EstabelecimentoDTO(e.id, e.nome, e.telefone, e.endereco)"
            ...
            countQuery =
                    "SELECT     COUNT(e.id)"
                            + " FROM    Estabelecimento e"
                            + " WHERE   (:#{#filtro.nome} IS NULL OR LOWER(e.nome) LIKE LOWER(CONCAT('%', :#{#filtro.nome}, '%')))")
    Page<EstabelecimentoDTO> listar(@Param("filtro") EstabelecimentoFilter filtro, Pageable pageable);
